...
$form = "<form action=\"#\" method=\"post\">
    ...
    <p>Config location without filename (IMPORTANT: OUT OF WEBROOT!)</p>
...
if(!isset($_POST['configureform'])){
    ...
    if(empty($_POST['loglocation']) || empty($_POST['uploaddir']) || empty($_POST['debuglevel']) || empty($_POST['loglocation']) || empty('configlocation')){
        ...
        if(!copy(__DIR__."/default_config", $_POST['configlocation'].'config.env')){
            ...
            $writer = file_put_contents($_POST['configlocation'].'config.env', $writedata, FILE_APPEND | LOCK_EX);
            header("Location: ?step3&config=".$_POST['configlocation'].'config.env');
        }
    }
}